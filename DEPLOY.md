...
| `SANDBOX_ENABLED` | `0` | Bash агента в SDK-пісочниці з маскуванням API-ключів |

### Rich messages та прогрес

| Змінна | За замовчуванням | Опис |
|--------|-----------------|------|
| `RICH_MESSAGES_ENABLED` | `1` | Структуровані rich-відповіді (Bot API 10.1); `0` — старий HTML/chunked шлях |
| `RICH_DRAFT_ENABLED` | `0` | Стрімінг чернетки rich-повідомлень; вмикається per-chat у `/settings` або цим env |
| `RICH_DRAFT_THROTTLE_MS` | `2000` | Інтервал оновлення чернетки, мс (мінімум 1000) |
| `PROGRESS_FULL_LOG` | `1` | Повний лог прогресу ланцюжком повідомлень; `0` — старе одне повідомлення з вікном 20 рядків |
| `GUEST_MODE_ENABLED` | `true` | Гостьовий режим (Bot API 10.0): відповіді на згадки в чатах, де бот не учасник |
